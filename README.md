...
это означает, что проблема возникает **до** обработки запроса приложением: на request stream HTTP/3 клиент отправил первым кадр, отличный от `HEADERS` (или некорректный HEADERS/QPACK блок).
